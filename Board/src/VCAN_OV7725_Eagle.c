...
uint8 ov7725_eagle_init(uint8 *imgaddr)
{
    ...
    DEBUG_PRINTF("\nOV7725 Config Success!");//显示ov7725配置成功
    return 0;
}
...
uint8 ov7725_eagle_reg_init(void)
{
    ...
    //DEBUG_PRINTF("\nGet ID success，SENSOR ID is 0x%x", Sensor_IDCode);//发送成功获取的信息
    //DEBUG_PRINTF("\nConfig Register Number is %d ", ov7725_eagle_cfgnum);//显示ov7725_eagle_cfgnum的数值
    ...
    //DEBUG_PRINTF("\nOV7725 Register Config Success!");//显示ov7725注册配置成功
    return 1;
}
