 ...
 * 定义 KEY 编号对应的管脚
 */
//PTXn_e KEY_PTxn[KEY_MAX] = {PTD10, PTD14, PTD11, PTD12, PTD7, PTD13, PTC14, PTC15};  这个注释的是原来的定义量
PTXn_e KEY_PTxn[KEY_MAX] = {PTD4, PTD10, PTD6, PTD8, PTE6, PTE4, PTD7, PTE2, PTE0};
...
KEY_e KEY_Scan(char mode)
{	 
	static char key_up=1;
	if(mode)key_up=1;		  
	if(key_up&&(key_get(KEY_U) ==  KEY_DOWN||key_get(KEY_D) ==  KEY_DOWN||key_get(KEY_L) ==  KEY_DOWN||key_get(KEY_R) ==  KEY_DOWN))
	{
		DELAY_MS(10);
		key_up=0;
		if(key_get(KEY_U) ==  KEY_DOWN)return KEY_U;
		else if(key_get(KEY_D) ==  KEY_DOWN)return KEY_D;
                else if(key_get(KEY_L) ==  KEY_DOWN)return KEY_L;
                else if(key_get(KEY_R) ==  KEY_DOWN)return KEY_R;
	}else if((key_get(KEY_U) ==  KEY_UP)&&(key_get(KEY_D) ==  KEY_UP)&&(key_get(KEY_L) ==  KEY_UP)&&(key_get(KEY_R) ==  KEY_UP))key_up=1; 	    
 	return KEY_No;
}
